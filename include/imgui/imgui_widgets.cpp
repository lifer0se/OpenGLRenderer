...
bool ImGui::BulletTextColored(ImVec4& color, ImVec4& color_hovered, ImVec4& color_selected, bool* p_selected, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    bool selected = BulletTextColoredV(color, color_hovered, color_selected, *p_selected, fmt, args);
    va_end(args);
	return selected;
}

// Text with a little bullet aligned to the typical tree node.
bool ImGui::BulletTextColoredV(ImVec4& color, ImVec4& color_hovered, ImVec4& color_selected, bool selected, const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
	...
	ImVec4 c = (selected) ? color_selected : (hovered || pressed) ? color_hovered : color;
	ImU32 col = ImGui::GetColorU32(IM_COL32(c.x * 255, c.y * 255, c.z * 255, c.w * 255));
...
bool ImGui::ImageRadioButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, bool active, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col, const ImVec4& border_col, const ImVec4& border_col_active, const ImVec4& border_col_hovered, const float& unselected_dim)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    // Render
    const ImU32 col = GetColorU32(active ? border_col_active : held || hovered ? border_col_hovered : border_col);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
	if (bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding, GetColorU32(bg_col));
	...
	ImVec4 c = tint_col;
	if (!active)
		c = ImVec4(tint_col.x * unselected_dim, tint_col.y * unselected_dim, tint_col.z * unselected_dim, tint_col.w);
    window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32(c));

    return pressed;
}

bool ImGui::ImageRadioCheckedButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, bool active, bool checked, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col, const ImVec4& border_col, const ImVec4& border_col_active, const ImVec4& border_col_hovered, const ImVec4& border_col_checked)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
	...
	float outer_frame_padding { 3.0f };
	ImVec2 min_padding { bb.Min.x - outer_frame_padding, bb.Min.y - outer_frame_padding };
	ImVec2 max_padding { bb.Max.x + outer_frame_padding, bb.Max.y + outer_frame_padding };
	ImVec4 n { 0, 0, 0, 0 };
	if (active)
		n = border_col_active;

    RenderNavHighlight(bb, id);
    RenderFrame(min_padding, max_padding, GetColorU32(n), true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));

    const ImU32 col = GetColorU32(checked ? border_col_checked : held || hovered ? border_col_hovered : border_col);
    RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
...
bool ImGui::ImageRadioButton(ImTextureID user_texture_id, const ImVec2& size, int* v, int v_button, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col, const ImVec4& border_col, const ImVec4& border_col_active, const ImVec4& border_col_hovered, const float& unselected_dim)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    const bool pressed = ImageRadioButtonEx(id, user_texture_id, size, *v == v_button, uv0, uv1, padding, bg_col, tint_col, border_col, border_col_active, border_col_hovered, unselected_dim);
    if (pressed)
			*v = v_button;
...
bool ImGui::ImageRadioCheckedButton(ImTextureID user_texture_id, const ImVec2& size, int* v, int v_button, int* c, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col, const ImVec4& border_col, const ImVec4& border_col_active, const ImVec4& border_col_hovered, const ImVec4& border_col_checked)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
	...
	const bool checked = *c == v_button;
    const bool pressed = ImageRadioCheckedButtonEx(id, user_texture_id, size, active, checked, uv0, uv1, padding, bg_col, tint_col, border_col, border_col_active, border_col_hovered, border_col_checked);
    if (pressed)
	{
		if (active)
			*c = v_button;
